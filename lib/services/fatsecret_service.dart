...
class FatSecretService {
  ...
  static const String _consumerKey =
      '344b078119a04ba18fb1bcbdd2f2aa9f'; // Buraya kendi anahtarınızı ekleyin
  static const String _consumerSecret =
      '045d4428e0544e2bbd5fc5ca5b17bad9'; // Buraya kendi anahtarınızı ekleyin
  ...
  String _generateSignature(Map<String, String> params, String httpMethod) {
    ...
    // Base string oluştur
    var baseString =
        '$httpMethod&${Uri.encodeComponent(_baseUrl)}&${Uri.encodeComponent(paramString)}';
  ...
  Future<FoodDetails> getFoodDetails(String foodId) async {
    var params = {'method': 'food.get', 'food_id': foodId};

    var response = await _makeApiCall(params);
    if (response['food'] == null) {
      throw Exception('Yemek detayı bulunamadı');
    }
...
class FoodDetails {
  ...
  factory FoodDetails.fromJson(Map<String, dynamic> json) {
    ...
    return FoodDetails(
      ...
      metricServingAmount:
          double.tryParse(
            serving['metric_serving_amount']?.toString() ?? '100',
          ) ??
          100.0,
      metricServingUnit: serving['metric_serving_unit']?.toString() ?? 'g',
    );
  }
}
